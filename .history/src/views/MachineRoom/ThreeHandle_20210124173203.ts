...
export default class ThreeHandle {
    ...
    ____generateObjAnnihilator(item: any) {
        let _this = this;
        var mtlLoader = new MTLLoader();
        mtlLoader.load(this.BASE_PATH + ('annihilator/annihilator.mtl'), function (materials) {
            materials.preload();
            var objLoader = new OBJLoader();
            objLoader.setMaterials(materials);
            objLoader.load(this.BASE_PATH + ('annihilator/annihilator.obj'), function (object: any) {
                item.childrens.forEach(function (childobj) {
                    var newobj = object.clone();
                    if (!newobj.objHandle) {
                        if (childobj.objHandle) {
                            newobj.objHandle = childobj.objHandle;
