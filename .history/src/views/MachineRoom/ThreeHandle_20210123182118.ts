...
export default class ThreeHandle {
    ...
    __generateEmptyCabinet(item: any) {
        let cabinet, floorHeight = 10, uuid = item.uuid || (Math.random() * Math.random() * Math.random()).toString().slice(2)
        ...
        tempObj.uuid = uuid
        tempObj.userData = item.data || { name: item.name, alarmInfo: [] }
        ...
        if (![null, undefined].includes(doors)) {
            ...
            if (doorSkins.length === 1 && doorname.length === 1) {
                ...
                /* 双门 */
                doorSkins.forEach((subSkin, subIndex) => {
        ...
        /* 机柜告警信息 */
        if (![null, undefined].includes(item.data)) {
            const { width: textAreaWidth = 70, height: textAreaHeight = 70, data: userData } = item
            const canvas = document.createElement('canvas')
             ...
             * @returns {any}
             */            
            function marker (context: CanvasRenderingContext2D,  x: number, y: number, text: string) {
    ...
    __generateWall(item: { style?: any; wallData?: any; depth?: any; height?: any; width?: any }) {
        /* 墙体的厚度、高度、宽度 */
     ...
     * @param {any} cube:THREE.Mesh<THREE.BoxGeometry
     * @param {any} any[]>
     * @param {any} newObj:THREE.Mesh<THREE.BoxGeometry
     * @param {any} any[]>
    ...
    __mergeModel(mergeOp: any, firstObj: THREE.Mesh<THREE.BoxGeometry, any[]>, secondObj: THREE.Mesh<THREE.BoxGeometry, any[]>, commonSkin?: any): THREE.Mesh<THREE.BoxGeometry, any[]> {
        ...
        switch (mergeOp) {
            ...
            case '&': {
                resultObjBSP = firstObjBSP.interspect(secondObjBSP)
            } break;
            default: {
        ...
        const result: THREE.Mesh<THREE.BoxGeometry, THREE.Material> = resultObjBSP.toMesh(cubeMaterialArray)
        ...
        resultFaces.forEach((resultFaceItem, resultFaceIndex) => {
            let faceset = false

            firstObjFace.forEach((firstFaceItem, firstFaceIndex) => {
                // const {} = resultFaceItem.vertexNormals[0]
                /* resultFaceItem.vertexNormals == firstFaceItem.vertexNormals*/
                const faceset = resultFaceItem.vertexNormals.every((subItem, subIndex) => {
    ...
    __generateHole(wallChilrenObj: { depth?: any; height?: any; width?: any; startDot?: any; endDot?: any; rotation?: any; uuid?: any; name?: any; skin?: any; objType?: any; skinColor?: any } | any) {
        /* 物体的厚度、高度、宽度 */
