...
export default class ThreeHandle {
    ...
    initRenderer(): any {
        ...
        var outlinePass = new OutlinePass(new THREE.Vector2(window.innerWidth, window.innerHeight), this.scene, this.camera, selectedObjects);
        ...
        outlinePass.hiddenEdgeColor.set(0x0000ff);
    ...
    onDocumentMouseMove(event) {
        ...
        if (intersects.length > 0) {
            ...
            this.outlinePass.selectedObjects = [intersects.reduce((prev, current) => {
                prev.push(current.object)

                return prev
            }, [])]

            // /* 机柜高亮 */
            // if (SELECTED.name.toString().indexOf("cabinet") != -1) {
