...
export default class ThreeHandle {
    ...
    __generateWall(item: any) {
        ...
        /* 建立墙面 */
        item.wallData.forEach((wallObj, index) => {
            ...
            if (![null, undefined].includes(childrens)) {
                childrens.forEach((wallChildren, index) => {
    ...
    __generateHole(wallObj: any) {
        /* 物体的厚度、高度、宽度 */
        const { depth: commonDepth = 40, height: commonHeight = 100, width: commonWidth = 300 } = wallObj
        ...
        const { height, rotation, uuid, name, skin, objType, skinColor } = wallObj
        const cubeObj = {
            width: wallWidth,
            height: height || commonHeight,
            depth: wallDepth,
