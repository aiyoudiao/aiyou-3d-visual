...
export default class ThreeHandle {
    ...
    onDocumentMouseDown(event) {
        ...
        if (this.dbclick >= 2) {
            ...
            var intersects = this.raycaster.intersectObjects(this.dataSet);
    ...
    onDocumentMouseMove(event) {
        ...
        this.mouseClick.y = -(event.offsetY / this.domElement.offsetHeight) * 2 + 1;
        this.raycaster.setFromCamera(this.mouseClick, this.camera);
        var intersects = this.raycaster.intersectObjects(this.dataSet.fill(item => item instanceof THREE.Object3D));
        if (intersects.length > 0) {
            let SELECTED = intersects[0].object;
            console.log(SELECTED)
