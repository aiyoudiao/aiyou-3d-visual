    ...
    events: {
        dbclick: [
            ...
            },
            {
                ...
                obj_event: function (_obj, _this) {
                    _this.openCabinetDoor(_obj, function (threeObj) { 
                        _obj = _obj.parent
                        let {x, y , z} = _obj.position

                        // console.log('threeObj.geometry.computeBoundingSphere()', _obj.geometry.computeBoundingSphere())
                        // console.log('threeObj.geometry.boundingSphere.center', _obj.geometry.boundingSphere.center)
                        // console.log('x,y,z', x, y ,z)
                        // // debugger
                        // threeObj.camera.position.x = 0;
                        // threeObj.camera.position.y = 0;
                        // threeObj.camera.position.z = 0;
                        // // threeObj.camera.lookAt(x, y, z)

                        // threeObj.orbitControls.target.x = x 
                        // threeObj.orbitControls.target.y = y
                        // threeObj.orbitControls.target.z = z
                        // threeObj.orbitControls.update()

                        /**
                         * TODO: 要实现点击门，切换到合适的位置，这个功能暂时只能通过点击门，切换到合适的方向，距离没法同步ok。
                         */

                        // threeObj.camera.position.copy(_obj.position)
                        // // threeObj.camera.position.z= _obj.children[1].geometry.boundingSphere.radius 
                        // threeObj.camera.position.y = threeObj.camera.position.y * 3
                        // threeObj.camera.position.x = Math.abs(threeObj.camera.position.x * 2)
                        // threeObj.orbitControls.target.copy(_obj.position)
                        // threeObj.orbitControls.update()


                        /**
                         * TODO: test 2021-02-11
                         */
                        // threeObj.camera.position.copy(_obj.position)
                        // threeObj.camera.position.y = threeObj.camera.position.y + 100

                        // console.log(' threeObj.camera.position.y * 3',  threeObj.camera.position.y * 3)
                        // threeObj.camera.position.x = threeObj.camera.position.x + 350
                        // console.log('threeObj.camera.position.x + 400', threeObj.camera.position.x + 400)

                        // // threeObj.camera.position.z = threeObj.camera.position.z + 50
                        // // console.log('threeObj.camera.position.z + 50', threeObj.camera.position.z + 50)
                        // console.log('threeObj.camera.rotation', threeObj.camera.rotation)


                        // // threeObj.camera.position.x = threeObj.camera.position.x * 2
                        // threeObj.orbitControls.target.copy(_obj.position)
                        // threeObj.orbitControls.update()
...
let cabinet = {
    ...
    objType: 'emptyCabinet',
    transparent: false,
    size: { width: 90, depth: 70, height: 250, thick: 2 },
    x: -350, y: 150, z: -220,
    rotation: [{ direction: 'y', degree: 0.5 * Math.PI }],
    style: {
        skin: {
...
for (var i = 0; i < 2; i++) {
    for (var j = 0; j < 6; j++) {
        ...
        for (let k = 0; k < obj.childrens.length; k++) {
            obj.childrens[k].userData.devid = obj.childrens[k].userData.devid + i + j;
            obj.childrens[k].userData.pointid = obj.childrens[k].userData.pointid + i + j;
        }
        obj.y = obj.y;
        obj.x = obj.x + 420 * i;
        obj.z = obj.z + 100 * j;
