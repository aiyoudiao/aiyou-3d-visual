...
import { isClickModel, findTopObj } from './ThreeTool'
...
export default class ThreeHandle {
    ...
    constructor(ThreeSet: { props: any; dataSet: any; eventList: any; sourcePath?: "./static/three.js/images/"; dataJson: any, vueModel: Vue | any }) {
        ...
        if (!WEBGL.isWebGLAvailable()) {
            ...
            document.getElementById(props.domID).appendChild(warning);
        } else {
    ...
    initScene(): any {
        ...
        const texture = textureLoader.load(this.BASE_PATH + 'back2.jpg')
    ...
    __generateText(floorWidth: any, floorHeight: any): any {
        ...
        marker(context, '上海南天', 'ip待分配', 500, 610)
    ...
    onDocumentMouseDown(event) {
        ...
        this.clickTimer = setTimeout(() => {
            ...
            if (this.dbclick === 2) {
                _this.customDBClick(event, _this)
            } else if (this.dbclick === 1) {
    ...
    flyTo(event) {
        ...
        if (intersects.length > 0) {
            ...
            if (intersects && isClickModel('cabinet', SELECTED)) {  
                ...
                const selectedCabinetDoor =  this.scene.getObjectByName(SELECTED.name + '&&' + 'cabinet_door') as any //findCabinetDoor(SELECTED)

                console.log(`this.scene.getObjectByName(SELECTED.name + '&&' + 'cabinet_door')`, this.scene.getObjectByName(SELECTED.name + '&&' + 'cabinet_door'))
                ...
                if (selectedCabinetDoor.doorState === 'open') {
                    this.openCabinetDoor(selectedCabinetDoor, () => {})
                }
                 ...
                 * 最终达到正好看到机柜的位置
                 */
                this.camera.position.copy(SELECTED.position)
                this.camera.position.y = this.camera.position.y + 60
                // this.camera.translateOnAxis(new THREE.Vector3(0, 1, 0).normalize(), 100)

                console.log(' this.camera.position.y * 3', this.camera.position.y * 3)
                this.camera.position.x = this.camera.position.x + 340

                console.log('this.camera.position.x + 400', this.camera.position.x + 400)

                // this.camera.position.z = this.camera.position.z + 50
                // console.log('this.camera.position.z + 50', this.camera.position.z + 50)
                 ...
                 * 让当前控制器的目标指向选中的对象
                 */
                this.orbitControls.target.copy(SELECTED.position)
                this.orbitControls.update()
    ...
    onDocumentMouseMove(event) {
        ...
        if (intersects.length > 0) {
            ...
            if (intersects && isClickModel('cabinet', SELECTED)) {
                ...
                this.vueModel.currentMesh.left = (_this.lastEvent.pageX + 10);
                this.vueModel.currentMesh.top = (_this.lastEvent.pageY + 10);
                this.vueModel.currentMesh.name = SELECTED.userData.name
                this.vueModel.currentMesh.show = true

                // this.scene.updateMatrixWorld(true)
            } else {
                this.vueModel.currentMesh.show = false
